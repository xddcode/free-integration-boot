## **seata详细的安装原理和部署细节请查看**
[SEATA安装与部署](https://blog.csdn.net/jixieguang/article/details/110621561)
...
```
>根据自己的环境修改 url、user、password 配置值，注意：如果使用了mysql8.0，要注意修改driverClassName 为：
com.mysql.cj.jdbc.Driver ，并且url后面要配置时区，否则启动seata会报错。
...
用户下单(`business-service`) -> 扣库存(`inventory-service`) -> 创建订单(`order-service`) -> 减少账户余额(`account-service`)

&nbsp;
## 测试前要
1.开启nacos  
2.开启seata服务，并且注册到nacos中  
3.依次启动上述服务（除公共工程不需要启动外，其余服务全都要启动）  
